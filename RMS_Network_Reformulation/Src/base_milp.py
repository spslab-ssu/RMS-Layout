...
def solve_base_milp(instance, config) -> RMSSolution:
    """기존 논문의 x-s-y implication formulation을 생성하고 푼다."""
    model = gp.Model(f"rms_base_{instance.problem_name}")
    ...
    x = model.addVars(x_keys, lb=0.0, ub=1.0, vtype=state_vtype, name="x")
    ...
    s = model.addVars(s_keys, lb=0.0, ub=1.0, vtype=state_vtype, name="s")
    y = model.addVars(y_keys, lb=0.0, ub=1.0, vtype=state_vtype, name="y")
    ...
    optimization_metadata: dict[str, Any] = {"lp_relaxation": lp_relaxation}
    if resource_capacity is not None:
